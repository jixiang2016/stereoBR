...
class DualRealDataset(Dataset):
    ...
    def get_seq(self, seq_name, data_mode):
        ...
        elif data_mode == 'stereoRS':
            seq_path = os.path.join(self.data_root, 'stereoRS', self.dataset_cls, seq_name)
            seq_mode_path = seq_path 
        elif data_mode == 'RSGR':
    ...
    def __getitem__(self, index):  
        ...
        imgs_arr = torch.from_numpy(imgs_arr.copy()).permute(2, 0, 1) 
        gts_arr = torch.from_numpy(gts_arr.copy()).permute(2, 0, 1)
        return torch.cat((imgs_arr, gts_arr), 0),cur_img_id,gts_ids 
